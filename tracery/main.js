...
function setup() {
  ...
  //CANVAS_WIDTH = 800;
  //CANVAS_HEIGHT = 608;
  CANVAS_WIDTH = max(800, windowWidth);
  CANVAS_HEIGHT = max(600, windowHeight);
  ...
  /// NPC: place quest givers
  let _test_npc = createCharacter(null,
    10, 10, "npc", "loiter", npcImg, 1, npc_grammar);
  npcSprites.add(_test_npc);
  npcSprites.add(new createCharacter("Mike the Fisherman", 50, 61, "npc", "loiter", npcImg, NUM_CHUNKS+TOWN_CHUNKS.FRILL, npc_grammar, true))

  /// NPC: place randos
  for (let _n = 0; _n < numGenericNPCs; _n++) {
    let _c = getRandomInteger(1, MAP_COLS - 1);
    let _r = getRandomInteger(1, MAP_ROWS - 1);
    let _chunk = getRandomInteger(0, NUM_CHUNKS);
    npcSprites.add(new createCharacter(null, _r, _c, "npc", "follow", npcImg, _chunk, npc_grammar));
  }
  ...
  //   if (_n == numGenericNPCs) { // campf"wire friend
  //     _r = _cf_row;
  //     _c = _cf_col;
...
// if clamping to 800x600, then we get weird border artifacts/offsets
